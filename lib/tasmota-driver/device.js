...
module.exports = class SonoffTasmotaDevice extends Homey.Device {
  async onInit() {
    this.log(`device init: name = ${ this.getName() }, id = ${ this.getDeviceId() }, module type = ${ this.getModule() }, version = ${ this.getVersion() }, topic = ${ this.getTopic() }`);
  ...
  async getIpAddress() {
    if (! this.ipAddress) {
      let result = await this.conn.sendCommand('status', '5').waitFor('status5.statusnet');
  ...
  async connect() {
    ...
    if (status === null) {
      // No status means we were unable to connect to the device (could have
      // been removed while we were waiting for a connection).
      this.log('unable to connect to device');
      return;
    }
