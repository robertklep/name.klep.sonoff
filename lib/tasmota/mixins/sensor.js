...
const { PWM_CHANNEL_MIN, PWM_CHANNEL_MAX }  = require('../constants');
...
module.exports.SensorDevice = superclass => class extends superclass {
  ...
  async onCapability(capability, value, opts) {
    ...
    if ([ 'light_hue', 'light_saturation', 'dim' ].includes(capability)) {
      let HSB = [
        360.0 * (capability === 'light_hue'        ? value : this.getCapabilityValue('light_hue')),
        100.0 * (capability === 'light_saturation' ? value : this.getCapabilityValue('light_saturation')),
        100.0 * (capability === 'dim'              ? value : this.getCapabilityValue('dim')),
      ];
      return this.conn.sendCommand('hsbcolor', HSB.join(','));
    }
    if (capability.startsWith('pwm')) {
      let pwmNr = /\.(\d)$/.exec(capability)[1];
      return this.conn.sendCommand('pwm' + pwmNr, String(value));
    }
    return super.onCapability(capability, value, opts);
  }
  ...
  async onDeviceCameOnline(status) {
    ...
    // Check PWM state through `state`
    let state = await this.conn.getState();
    if ('PWM' in state) {
      this.processPwmData(state.PWM);
    }
    ...
    return super.onDeviceCameOnline(status);
  }
  ...
  processPwmData(payload) {
    for (let i = PWM_CHANNEL_MIN; i <= PWM_CHANNEL_MAX; i++) {
      let key = `PWM${ i }`;
      if (key in payload) {
        this.setCapabilityValue(`pwm.${ i }`, Number(payload[key]));
      }
    }
  }
