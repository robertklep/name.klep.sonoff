...
const { generateMobileCard } = require('./capabilities');
...
module.exports = class SonoffTasmotaDriver extends Homey.Driver {
  ...
  onPair(socket) {
    ...
    socket.on('pair.init', (data, callback) => {
      ...
      let modules = await this.supportedModules(device);
      if (! modules.includes(info1.Module)) {
      ...
      let capabilities = await this.supportedCapabilities(device);
      ...
      this.log('generated mobile card:', JSON.stringify(generateMobileCard(capabilities)));
      ...
      return callback(null, [
        this.configureNewDevice({
          ...
          mobile : generateMobileCard(capabilities),
          capabilities
        })
      ]);
    }).on('error', err => {
